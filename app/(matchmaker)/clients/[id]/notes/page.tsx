...
export const metadata = { title: "Notes | Matchmaker Portal" };

import { createClient } from "@/lib/supabase/server";
import { redirect } from "next/navigation";
import Link from "next/link";
import { SupportNotesManager } from "@/components/matchmaker/support-notes-manager";
...
export default async function NotesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch all support notes for this client, joined with author profile
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        ...
        <h1 className="text-3xl font-heading font-bold text-gold tracking-tight mt-2">Support Notes</h1>
        <p className="text-on-surface-variant text-sm mt-1">Internal notes and updates for {clientName}</p>
      </div>
